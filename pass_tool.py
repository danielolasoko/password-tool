...
class PasswordTool:
    ...
    def create_widgets(self):
        tk.Label(self.root, text="Welcome to Password Tool", font=("Arial", 16)).pack(pady=10)

        # Display options for the user
        tk.Label(self.root, text="Select an option:", font=("Arial", 12)).pack(pady=5)
        tk.Button(self.root, text="1. Generate Strong Password", command=self.generate_password, width=30).pack(pady=5)

        tk.Button(self.root, text="2. Hash and Salt Password", command=self.hash_and_salt_password, width=30).pack(pady=5)

        self.result_label = tk.Label(self.root, text="", font=("Arial", 12))
        self.result_label.pack(pady=5)

        tk.Button(self.root, text="Quit", command=self.root.quit, width=30).pack(pady=5)
    ...
    def generate_password(self):
        ...
        tk.Button(self.root, text="Copy Password", command=lambda: self.copy_to_clipboard(generated_password)).pack(pady=5)
    ...
    def hash_and_salt_password(self):
        ...
        if password:
            ...
            tk.Button(self.root, text="Copy Salt", command=lambda: self.copy_to_clipboard(salt)).pack(pady=5)
            tk.Button(self.root, text="Copy Hash", command=lambda: self.copy_to_clipboard(hashed_password)).pack(pady=5)
        else:
